   // --- Elementos do DOM (Pesquisa e Filtro) ---
    ...
    const secaoEventos = document.querySelectorAll('.caixa-categoria')[0];
    

    const mapaSecoes = { evento: secaoEventos};
    let filtroAtual = 'todos';

    // --- CONFIGURAÇÃO INICIAL: OCULTAR TODAS AS SEÇÕES ---
    function ocultarTodasSecoes() {
        [secaoEventos].forEach(s => { 
    ...
    function realizarPesquisa() {
        ...
        if (filtroAtual === 'todos') {
            secoesAlvo = [ 
                ...
                { secao: secaoEventos, tipo: 'evento' }
            ];
        } else {
...
const batePesquisa =
  termo === '' || textoItem.includes(termo);

const bateFiltro =
  filtroAtual === 'todos' ||
  filtroAtual === statusItem;

const bate = batePesquisa && bateFiltro;
